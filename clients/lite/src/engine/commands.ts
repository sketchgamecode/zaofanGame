...
export type LogType = 'cmd' | 'ok' | 'err' | 'info' | 'reward' | 'battle';
export type LogLine = { id: string; type: LogType; text: string };
export type CmdResult = { state: GameState; logs: LogLine[] };

const uid = () => Math.random().toString(36).slice(2, 8);
const mk = (type: LogType, text: string): LogLine => ({ id: uid(), type, text });

function getToday() { return new Date().toISOString().slice(0, 10); }

function makePlayerCombatant(gs: GameState): Combatant {
...
function genNPC(playerLevel: number, prestige: number): Combatant & { prestige: number } {
...
function applyXP(state: GameState, xp: number): GameState {
...
export function executeCommand(input: string, state: GameState): CmdResult {
  ...
  const say = (type: LogType, text: string) => logs.push(mk(type, text));
  ...
  switch (cmd) {
    ...
    // ── MISSION ────────────────────────────────────────────────────────────
    case 'mission': case 'm': {
      if (s.activeMission) { say('err', '已有进行中的任务，先领取或跳过'); break; }
      const tpl = MISSION_TEMPLATES.find(t => t.type.toLowerCase() === arg.toUpperCase() || arg === t.type.toLowerCase());
      if (!tpl) { say('err', `用法: mission a / b / c`); break; }
      if (s.resources.rations < tpl.food) { say('err', `干粮不足 (需要${tpl.food})`); break; }
      ...
      s = { ...s, resources: { ...s.resources, rations: s.resources.rations - tpl.food },
        activeMission: { id: `m_${Date.now()}`, type: tpl.type as any, name: tpl.name, durationSec: tpl.min * 60, foodCost: tpl.food, expReward, coinReward, dropRate: 0.1, endTime } };
    ...
    case 'collect': case 'c': {
      ...
      if (s.playerLevel > state.playerLevel) say('ok', `🎉 升级! Lv.${state.playerLevel} → Lv.${s.playerLevel}`);
      break;
    }

    // ── SKIP ──────────────────────────────────────────────────────────────
    case 'skip': {
      if (!s.activeMission) { say('err', '当前没有任务'); break; }
      if (s.resources.tokens < 1) { say('err', '通宝不足 (需要1💎)'); break; }
      const m = s.activeMission;
    ...
    case 'fight': case 'f': {
      ...
      if (cdEnd > Date.now()) {
        ...
        say('err', `冷却中 (${Math.floor(secs / 60)}:${(secs % 60).toString().padStart(2, '0')})`); break;
      }
      ...
      if (result.pWin) {
        if (xpLeft > 0) xpGain = Math.floor((XP_TABLE[s.playerLevel] ?? 400) * 0.05);
      ...
      if (s.playerLevel > state.playerLevel) say('ok', `🎉 升级! Lv.${s.playerLevel}`);
      break;
    }
    ...
    case 'boss': {
      ...
      for (const ch of DUNGEON_CHAPTERS) {
        ...
        if (beaten < ch.bosses.length) {
          ...
          const newProg = { ...prog };
          if (result.pWin) newProg[ch.id] = beaten + 1;
          s = applyXP({
          ...
          if (result.pWin) say('reward', `🏆 讨伐成功! +${boss.rewardXp}XP +${boss.rewardCoins}🪙`);
          else say('err', `💀 败于「${boss.name}」`);
          if (s.playerLevel > state.playerLevel) say('ok', `🎉 升级! Lv.${s.playerLevel}`);
          found = true; break;
        }
      }
      if (!found) say('ok', '🏆 所有副本已通关！');
      break;
    }

    // ── UPGRADE ───────────────────────────────────────────────────────────
    case 'upgrade': case 'up': case 'u': {
      const MAP: Record<string, keyof typeof s.attributes> = {
      ...
      const attr = MAP[arg];
      if (!attr) { say('err', '用法: up str/agi/int/con/lck'); break; }
      const cur = s.attributes[attr];
      const cost = MathCore.getUpgradeCost(cur);
      if (s.resources.copper < cost) { say('err', `铜钱不足 (需要${cost}🪙，当前${s.resources.copper})`); break; }
      s = { ...s, attributes: { ...s.attributes, [attr]: cur + 1 }, resources: { ...s.resources, copper: s.resources.copper - cost } };
      const NAMES: Record<string, string> = { strength: '武力', agility: '身法', intelligence: '智谋', constitution: '体质', luck: '福缘' };
