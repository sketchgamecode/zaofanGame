...
本次版本按当前试玩反馈与太阁立志传式场景结构纠偏：

1. 顶层 `SceneViewport` 回到完整 `1920x1080`，场景吃满主舞台。
2. 原右侧常驻 `RightRail` 改为左上随身状态栏，只承载个人入口，不占用主场景核心视野。
3. 京城 / 城市 / 场所采用分层入口：全国地图可有坐标地标；进入城市或场所后，入口统一放在底部横向卡片栏。
4. 城市场所不再手工摆建筑热点；场所内 NPC / 服务也不再按背景图逐个对位。
5. 美术生产默认使用统一横向 landscape 背景图；入口卡片可复用背景图或使用专门卡面图。
...
3. 当前已演进为左上随身状态栏，只保留个人随身功能，例如角色头像入口、资源、随身行囊、战报/邮件。
...
本次版本曾将顶层框架从“右栏 + 底栏”修正为“右栏常驻 + 左侧场景满高”：

1. 移除顶层 `BottomHud`，避免底部 100px 常驻栏影响 Shakes & Fidget 式系统界面坐标复刻。
2. `SceneViewport` 曾改为 `1534x1080`，左侧场景可吃满舞台高度。
...
4. 明确 `CityScene` 地标是角色扮演玩法入口；随身栏只保留个人管理与随身功能，不再作为第二套玩法地图。

### V1.0 - 2026-05-10

初版定义固定 `1920x1080` 舞台、常驻栏、`SceneViewport` 场景切换与基础复用组件方向。
...
2. 登录成功后，主场景吃满舞台；个人功能收进左上随身状态栏。
3. 系统切换只替换主场景内容，不替换主框架。
...
这套前端不是常规网页，而是“固定舞台 + 可替换场景层 + 左上随身状态栏”。
...
左上随身状态栏不负责差事、商店、副本、竞技、情报等角色扮演行为入口；这些入口必须落在地图、城市、场所和场所任职 NPC 上。
...
| Zone | X | Y | W | H |
| :--- | ---: | ---: | ---: | ---: |
| `SceneViewport` | 0 | 0 | 1920 | 1080 |
| `PersonalStatusBar` | 14 | 14 | 自适应 | 82 |

左上随身状态栏内部：

| Zone | 说明 |
| :--- | :--- |
| `AvatarButton` | 小头像入口，点击进入 `InventoryScene` |
| `ResourcePanel` | 铜钱、令牌、沙漏、声望等随身资源 |
| `QuickNav` | 战报、随身行囊、邮件等个人功能 |
...
当前项目已移除大面积右侧常驻导航栏。后续不得把玩法入口重新塞回右侧栏；如果需要个人快捷入口，应加入左上随身状态栏。
...
1. 默认进入 `CityScene`。
2. `CityScene` 表达当前城市或京城场景，不再给每个建筑手工对位热点。
3. 角色扮演行为必须从地图地标、底部场所卡片、场所 NPC 或场所职务进入。
4. 左上随身状态栏只允许切换个人管理与随身功能，不允许新增差事、商店、副本、竞技等玩法场所入口。
...
### 4.0.1 Map / City / Location Layer Rule

入口层级采用以下固定结构：

1. `WorldMapScene` / 大明版图层：唯一允许大量手工坐标地标的层级。城市、边镇、重点区域可以放在真实地图背景上对位。
2. `CityScene` / 城市层：使用一张横向 landscape 背景图。城市内可进入的衙门、店铺、营房、街市等，统一显示为底部横向 `LocationCard`，不在背景图上手工摆建筑热点。
3. `LocationScene` / 场所层：使用一张横向 interior / exterior 背景图。若场所内还有子场所，继续使用底部横向卡片；若没有子场所，则底部显示任职角色卡和服务牌。
4. `ServiceScene` / 子玩法层：差事、商店、副本、竞技、补给等具体玩法页，必须显示来源场所和承办角色，并提供“返回场所”。

这条规则的目的，是把需要对位置的工作收缩到大明版图一层。城市和场所只换背景图与卡片数据，不因美术重新构图而重新调整热点坐标。

V1.5 当前城市分配：

| 城市 | 定位 | 当前承载场所 |
| :--- | :--- | :--- |
| 顺天府 | 京师 | 皇城、吏部衙门、北镇抚司、神机营、都察院、国公府、京城酒楼、城门包子铺、教司坊 |
| 辽阳 | 九边军镇 | 九边都司 |
| 扬州府 | 两淮盐引 | 盐商会馆 |
| 应天府 | 留都旧制 | 江南织造局 |
| 山海关 | 蓟辽咽喉 | 山海关守御署、关城、塘报、天下第一关预告 |
| 嘉峪关 | 河西锁钥 | 嘉峪关守备营、烽燧、军屯、关城楼预告 |
| 济南府 | 山东布政司治所 | 三司署、山东都司营、客栈、趵突泉预告 |
| 太原府 | 山西布政司治所 | 三司署、山西都司营、客栈、晋祠预告 |
| 开封府 | 河南布政司治所 | 三司署、河南卫所营、客栈、大相国寺预告 |
| 西安府 | 陕西布政司治所 | 三司署、陕西都司营、客栈、钟鼓楼预告 |
| 杭州府 | 浙江布政司治所 | 三司署、浙江都司营、客栈、西湖预告 |
| 南昌府 | 江西布政司治所 | 三司署、江西都司营、客栈、滕王阁预告 |
| 武昌府 | 湖广布政司治所 | 三司署、湖广都司营、客栈、黄鹤楼预告 |
| 成都府 | 四川布政司治所 | 三司署、四川都司营、客栈、武侯祠预告 |
| 福州府 | 福建布政司治所 | 三司署、福建水师营、客栈、三坊七巷预告 |
| 广州府 | 广东布政司治所 | 三司署、广东水师营、客栈、镇海楼预告 |
| 桂林府 | 广西布政司治所 | 三司署、广西都司营、客栈、象鼻山预告 |
| 云南府 | 云南布政司治所 | 三司署、云南都司营、客栈、金马碧鸡坊预告 |
| 贵阳府 | 贵州布政司治所 | 三司署、贵州都司营、客栈、甲秀楼预告 |

说明：

1. 第一版只做“场所主归属城市”，同一真实 `locationId` 不重复出现在多座城市。
2. 省会的三司署、军事场所、客栈和名胜目前是前端世界观预告卡，不接服务端玩法，不写入存档。
3. 后续若要表现分号、暗线、分支门路，应新增子场所或场所变体，不要直接复制同一个 `locationId`。

### 4.0.2 Location Card Rule

`LocationCard` 是城市和场所层的主要入口表现：

1. 固定横向滚动排列在画面底部。
2. 卡片上部显示场所插图；没有专门卡面图时，可复用该场所背景图。
3. 卡片文字只保留名称、所属势力、状态和一句功能摘要。
4. 卡片点击进入对应场所或子场所。
5. 卡片不得承担复杂操作；复杂信息放进场所详情弹窗、主官面板、黄册或近日报告。

### 4.0.3 Art Production Rule

当前推荐美术规格：

1. 大明版图背景：`1920x1080`，用于全国地图，允许城市地标手工对位。
2. 城市背景：`1920x1080` 或同等 16:9 横向图，例如 `bg_city_beijing.png`。
3. 场所背景：`1920x1080` 或同等 16:9 横向图，例如 `bg_location_northern_bureau.png`。
4. 场所卡面：可选，建议 16:9 或 4:3，例如 `card_location_northern_bureau.png`；没有时复用背景图。
5. NPC 不要求单独在场景里对位摆放。NPC 以 `CharacterPortraitCard` 出现在底部职务区、弹窗或服务页。
...
4. `CityScene` 底部场所卡片和场所 NPC 职务按钮负责承接商店入口；左上随身状态栏不再新增商店快捷入口。
5. 从场所进入商店时，商店应显示经手 NPC 的 `CharacterPortraitCard`，并在购买、出售、换货后给出角色化反馈。

Shop Scene 坐标蓝图（V1.3 历史 CSS 值，基于旧 `SceneViewport 1534x1080`，当前仅作旧实现参考）：
...
2. 点击左上随身状态栏的角色头像入口，应进入 `InventoryScene`。
...
3. 曾为右侧随身栏与 `CityScene` 新增两个店铺入口；当前已纠偏为仅由 `CityScene` 场所职务承接店铺入口，随身栏不再承载店铺。
...
5. 当前纳入 CSS 同源约束的模块包括：`CharacterPortraitCard`、`ResourceBadge` / `PlayerResourcePanel`、`ItemSlot` / `DraggableItemSlot` / `ItemDragPreview`、左上随身栏导航按钮。后续新增 token、resource、panel 类复用模块默认也遵守此规则。
